...
<table>
	...
	<tbody>
		<tr>
			...
			<td>지정한 내장객체에 name과 o를 연결하여 저장한다</td>
		</tr>
		<tr>
			<td>Object 내장객체.setAttribute(String name, Object o)</td>
			<td>지정한 name으로 객체 o를 반환받는다</td>
		</tr>
		<tr>
			<td>void 내장객체.removeAttribute(String name_)</td>
...
%>
<%--표현식이나 out.print() 를 이용하면 다운캐스팅 없이 문자열 형태로 출력된다 --%>
<h3>test1 : <%=pageContext.getAttribute("test1") %></h3>
<h3>test2 : <%=test2 %></h3>
<h3>test3 : <%=test3 %></h3>
<h3>test4.split(" ")[0] : <%=((String)application.getAttribute("test4")).split(" ")[0] %></h3>
<h3>test : <%=test %></h3>

<fieldset>
	<p><a href="ex02-test1.jsp"><button>test1</button></a>
</fieldset>
