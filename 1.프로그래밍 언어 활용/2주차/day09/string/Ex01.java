...
public class Ex01 {
	public static void main(String[] args) {
		...
		System.out.printf("s2 : %s, s3 : %s%n", s2, s3);

		// compareTo를 활용한 정렬 코드

		String[] arr4 = { "banana", "apple", "candy" };
		...
		// A.endsWith(B) : A 문자열이 B내용으로 끝나면 true
		// A.contains(B) : A문자열이 B내욜을 포함하면 true
		// A.equals(B) : 대소문자 무시하고 A문자열이 B내용과 일치하면 true

		boolean b1 = "Hello".startsWith("He");	// 전화번호는 010- 으로 시작해야 한다
		boolean b2 = "Hello".endsWith("lo");	// 파일이름이 .jpg인지 확인한다
		boolean b3 = "Hello".contains("Hell");	// 이름에 '호'를 포함하는 모든 결과 조회
		boolean b4 = "Hello".equals("hello");	// 이름이 일치하는 상품 검색
		boolean b5 = "Hello".equalsIgnoreCase("hello");	// 명령어 대소문자 구분없이 실행할 때
		
		System.out.println("Hello.startsWith(He) : " + b1);
		System.out.println("Hello.endsWith(lo) : " + b2);
		System.out.println("Hello.contatins(Hell) : " + b3);
